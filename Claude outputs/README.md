...
1. [github.com](https://github.com) 가입 → 우측 상단 **+** → **New repository**
2. 이름: `insta-threads-auto` (아무거나)
3. **Public 으로 생성** — 중요합니다.
   - 미디어 파일을 CDN 공개 주소로 Meta 서버에 넘겨야 해서 Public 이어야 합니다.
   - 어차피 발행할 콘텐츠라 공개돼도 문제없고, **토큰은 Secrets 에 암호화 저장**되므로 Public 이어도 안전합니다.
4. 이 폴더의 파일을 저장소에 올립니다.
   - 저장소 화면 → **Add file → Upload files** → `README.md` `queue.csv` `.gitignore`
     `scripts` 폴더 `media` 폴더를 드래그 → Commit
   - **`workflows` 폴더의 yml 두 개는 경로를 바꿔서 넣어야 합니다.**
     저장소 → **Add file → Create new file** → 파일명 칸에
     `.github/workflows/daily-post.yml` 을 그대로 입력(슬래시를 치면 폴더가 자동 생성) →
     내용 붙여넣고 Commit. `monthly-refresh.yml` 도 같은 방식으로 반복합니다.
     (구글 드라이브가 점으로 시작하는 폴더를 막아서 임시로 `workflows/` 에 둔 것입니다)
